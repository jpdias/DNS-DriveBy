...
{
    uint32_t placeMultiplier = 1;
    for (uint8_t i = 1; i < placeValue; i++)
    {
        placeMultiplier *= 100;
    }
...
uint8_t getGPS()
{
    ...
    uint8_t mon = getPlaceValue(date, 3);
    uint8_t day = getPlaceValue(date, 2);
    uint8_t year = getPlaceValue(date, 1);
...
bool dnsRequest(char *dataB32, uint8_t lenDataB32)
{
    ...
    if (WiFi.hostByName(gpsURL, resolvedIP))
    {
